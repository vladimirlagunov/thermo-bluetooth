...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(forName: UIScreen.brightnessDidChangeNotification, object: nil, queue: nil,
                using: { (v) in self.handleScreenBrightness((v.object as! UIScreen).brightness.native) })
        handleScreenBrightness(UIScreen.screens[0].brightness.native)
    }
    ...
    private func handleScreenBrightness(_ brightness: Double) {
        let bg: UIColor
        let fg: UIColor
        if brightness < 0.25 {
            bg = .black
